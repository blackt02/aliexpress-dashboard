...
# ── Auto token refresh (runs every page load) ─────────────────────────────────
...
with st.sidebar:
    ...
    if tok:
        ...
        with st.expander("🔑 Token Status", expanded=False):
            st.success(f"Token: `{tok[:20]}…`")
            if exp:
                import time as _t

                try:
                    remaining_days = (int(exp) - int(_t.time() * 1000)) / (
                        86_400 * 1000
                    )
                    st.caption(f"⏳ Expires in ~{remaining_days:.0f} days")
                except Exception:
            ...
            with col_t:
                if st.button("🔌 Test", use_container_width=True):
                    ...
                    ok, msg = AliExpressAPI(
                        config["app_key"], config["app_secret"], tok
                    ).test_connection()
        ...
        st.error("⚠️ No access token in config.json")

    st.divider()
    st.markdown("### 🔍 Filters")

    preset = st.selectbox(
        "Quick range", ["Today", "Yesterday", "Last 7 days", "Last 30 days", "Custom"]
    )
    today = date.today()
    if preset == "Today":
...
if refresh_clicked:
    ...
    if not _token:
        st.error("❌ No access token in config.json — please update it manually.")
    else:
        from api_client import AliExpressAPI
        ...
        try:
            ...
            orders = api.fetch_all_orders(
                start_str, end_str, progress_callback=_progress
            )
...
date_label = (
    str(_dates[0]) if _dates[0] == _dates[-1] else f"{_dates[0]}  →  {_dates[-1]}"
)
...
if df.empty:
    if not config.get("access_token"):
        st.warning("⚠️ No access token found in config.json.")
    else:
...
st.divider()
st.caption(
    "AliExpress Affiliate Dashboard  •  Data stored locally in orders.db  •  Auto-refresh: 15:00 & 16:00"
)
