...
def _sign(secret: str, params: dict) -> str:
    ...
    return hmac.new(secret.encode("utf-8"), sign_str.encode("utf-8"), hashlib.sha256).hexdigest().upper()
...
def _parse_order(raw: dict) -> dict:
    return {
        ...
        "commission_rate":               float(str(raw.get("commission_rate", "0")).replace("%", "") or 0),
        "completed_payments_amount":     float(raw.get("paid_amount", 0) or 0) / 100,
        "estimated_payments_commission": float(raw.get("estimated_paid_commission", 0) or 0) / 100,
...
def get_auth_url(app_key: str, redirect_uri: str = "https://besttopzone.com/") -> str:
    return (f"https://api-sg.aliexpress.com/oauth/authorize"
...
def exchange_code_for_token(app_key: str, app_secret: str, auth_code: str) -> dict:
    ...
    params    = {"app_key": app_key, "timestamp": str(int(time.time() * 1000)), "sign_method": "sha256", "code": auth_code}
    sign_str  = api_path + "".join(f"{k}{v}" for k,v in sorted(params.items()))
    params["sign"] = hmac.new(app_secret.encode(), sign_str.encode(), hashlib.sha256).hexdigest().upper()
    resp = requests.post("https://api-sg.aliexpress.com/rest" + api_path, data=params,
...
def refresh_access_token(app_key: str, app_secret: str, refresh_token: str) -> dict:
    ...
    params    = {"app_key": app_key, "timestamp": str(int(time.time() * 1000)), "sign_method": "sha256", "refresh_token": refresh_token}
    sign_str  = api_path + "".join(f"{k}{v}" for k,v in sorted(params.items()))
    params["sign"] = hmac.new(app_secret.encode(), sign_str.encode(), hashlib.sha256).hexdigest().upper()
    resp = requests.post("https://api-sg.aliexpress.com/rest" + api_path, data=params,
                         headers={"Content-Type": "application/x-www-form-urlencoded;charset=utf-8"}, timeout=30)
...
class AliExpressAPI:
    ...
    def _post(self, extra: dict) -> dict:
        params = _base_params(self.app_key, self.access_token, extra.pop("method", "aliexpress.affiliate.order.list"))
        params.update(extra)
    ...
    def fetch_page(self, start_time: str, end_time: str, status: str = "Payment Completed", page_no: int = 1) -> Tuple[list, int]:
        data = self._post({
        ...
        result = data.get("aliexpress_affiliate_order_list_response", {}).get("resp_result", {})
        if result.get("resp_code") not in (200, "200"):
            raise ValueError(f"API error {result.get('resp_code')}: {result.get('resp_msg')}")
    ...
    def fetch_all_orders(self, start_time: str, end_time: str, status: str = "Payment Completed",
                         progress_callback=None) -> list:
        from datetime import datetime, timedelta
        # Split into 7-day chunks to avoid API limit
        fmt = "%Y-%m-%d %H:%M:%S"
    ...
    def fetch_by_ids(self, order_ids: list) -> list:
        ...
        for i in range(0, len(order_ids), 50):
            ...
            data  = self._post({"method": "aliexpress.affiliate.order.get", "order_ids": ",".join(str(x) for x in chunk)})
            result = data.get("aliexpress_affiliate_order_get_response", {}).get("resp_result", {})
